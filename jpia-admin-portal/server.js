      ...
      users: [
        {
          id: 'user-superadmin',
          name: 'Super Admin',
          email: 'superadmin@jpia.local',
          passwordHash: hashPassword('SuperAdmin#2026!', superSalt),
          salt: superSalt,
          role: 'super_admin',
          status: 'active',
          emailVerified: true,
    ...
    readBody(req)
      .then((body) => {
        ...
        if (user.status !== 'active') {
          res.statusCode = 403;
          res.end(JSON.stringify({ message: 'Your account is suspended or inactive.' }));
          return;
        }
  ...
  if (pathname.startsWith('/api/users/')) {
    ...
    if (req.method === 'PATCH') {
      if (user.role !== 'super_admin') {
        res.statusCode = 403;
        res.end(JSON.stringify({ message: 'Only the Super Admin can modify user roles or accounts.' }));
        return;
      }
...
server.listen(PORT, HOST, () => {
  ...
  console.log('Demo Super Admin credentials: superadmin@jpia.local / SuperAdmin#2026!');
